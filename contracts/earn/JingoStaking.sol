 ...
 * @notice JingoStaking is a contract that allows JGO deposits and receives JGO sent by FeeCollector's
 * harvests. Users deposit JGO and receive a share of what has been sent by FeeCollector based on their participation of
 * the total deposited JGO. It is similar to a MasterChef, but we allow for claiming of different reward tokens
 ...
 */
contract JingoStaking is Ownable {
    ...
    struct UserInfo {
        ...
        /**
         * @notice We do some fancy math here. Basically, any point in time, the amount of JGO'
         * entitled to a user but is pending to be distributed is:
         *
         *   pending reward = (user.amount * accRewardPerShare) - user.rewardDebt[token]
         *
         * Whenever a user deposits or withdraws JGO. Here's what happens:
    ...
    /// @dev Internal balance of JGO, this gets updated on user deposits / withdrawals
    /// this allows to reward users with JGO
    uint256 public internalJGOBalance;
    ...
    /// @dev Info of each user that stakes JGO
    mapping(address => UserInfo) private userInfo;

    /// @notice Emitted when a user deposits JGO
    ...
    /// @notice Emitted when a user withdraws JGO
    ...
    /// @notice Emitted when a user emergency withdraws its JGO
    ...
    /**
     * @notice Constructor of JingoStaking contract
     * @dev This contract needs to receive an ERC20 `_rewardToken` in order to distribute them
     * (with FeeCollector in our case)
     * @param _rewardToken The address of the ERC20 reward token
     * @param _JGO The address of the JGO token
    ...
    constructor(
        ...
        require(
            ...
            "JingoStaking: reward token can't be address(0)"
        );
        require(
            address(_JGO) != address(0),
            "JingoStaking: JGO can't be address(0)"
        );
        require(
            _feeReceiver != address(0),
            "JingoStaking: fee collector can't be address(0)"
        );
        require(
            _depositFeePercent <= 5e17,
            "JingoStaking: max deposit fee can't be greater than 50%"
        );
    ...
    /**
     * @notice Deposit JGO for reward token allocation
     * @param _amount The amount of JGO to deposit
     */
    function deposit(uint256 _amount) external {
        ...
        internalJGOBalance = internalJGOBalance.add(_amountMinusFee);
        JGO.safeTransferFrom(_msgSender(), feeReceiver, _fee);
        JGO.safeTransferFrom(_msgSender(), address(this), _amountMinusFee);
     ...
     * @return The amount of JGO user has deposited
    ...
    function addRewardToken(IERC20 _rewardToken) external onlyOwner {
        require(
            ...
            "JingoStaking: token can't be added"
        );
        require(
            rewardTokens.length < 25,
            "JingoStaking: list of token too big"
        );
        rewardTokens.push(_rewardToken);
        isRewardToken[_rewardToken] = true;
    ...
    function removeRewardToken(IERC20 _rewardToken) external onlyOwner {
        require(
            ...
            "JingoStaking: token can't be removed"
        );
        updateReward(_rewardToken);
    ...
    function setFeeReceiver(address _feeReceiver) external onlyOwner {
        require(
            ...
            "JingoStaking: fee receiver can't be the same"
        );
    ...
    function setDepositFeePercent(
        ...
        require(
            _depositFeePercent <= 5e17,
            "JingoStaking: deposit fee can't be greater than 50%"
        );
    ...
    function pendingReward(
        ...
        require(isRewardToken[_token], "JingoStaking: wrong reward token");
        UserInfo storage user = userInfo[_user];
        uint256 _totalJGO = internalJGOBalance;
        uint256 _accRewardTokenPerShare = accRewardPerShare[_token];

        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == JGO
            ? _currRewardBalance.sub(_totalJGO)
            : _currRewardBalance;

        if (_rewardBalance != lastRewardBalance[_token] && _totalJGO != 0) {
    ...
    /**
     * @notice Withdraw JGO and harvest the rewards
     * @param _amount The amount of JGO to withdraw
     */
    function withdraw(uint256 _amount) external {
        ...
        require(
            ...
            "JingoStaking: withdraw amount exceeds balance"
        );
        ...
        internalJGOBalance = internalJGOBalance.sub(_amount);
        JGO.safeTransfer(_msgSender(), _amount);
    ...
    function updateReward(IERC20 _token) public {
        ...
        // Did JingoStaking receive any token
        if (_rewardBalance == lastRewardBalance[_token] || _totalJGO == 0) {
            return;
        }
        ...
        accRewardPerShare[_token] = accRewardPerShare[_token].add(
            _accruedReward.mul(ACC_REWARD_PER_SHARE_PRECISION).div(_totalJGO)
        );
    ...
    function safeTokenTransfer(
        ...
        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == JGO
            ? _currRewardBalance.sub(internalJGOBalance)
            : _currRewardBalance;
